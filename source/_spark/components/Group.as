package spark.components
{
   ...
   public class Group extends GroupBase implements IVisualElementContainer, ISharedDisplayObject
   {
      ...
      private function setMXMLContent(param1:Array) : void
      {
         ...
         {
            _loc2_ = int(this._mxmlContent.length - 1);
            while(_loc2_ >= 0)
            {
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         var _loc7_:ISharedDisplayObject = null;
         var _loc8_:int = 0;
         ...
         if(this.numGraphicElements > 0)
         {
            _loc4_ = this.numElements;
            _loc5_ = 0;
            while(_loc5_ < _loc4_)
            {
               _loc6_ = this.getElementAt(_loc5_) as IGraphicElement;
               if(_loc6_)
               {
                  if(_loc6_.depth == 0)
                  {
                     if(_loc6_.displayObjectSharingMode != DisplayObjectSharingMode.USES_SHARED_OBJECT)
                     {
                        if(_loc3_)
                        {
                           _loc3_.redrawRequested = false;
                        }
                        _loc3_ = _loc6_.displayObject as ISharedDisplayObject;
                     }
                     if(!_loc3_ || _loc3_.redrawRequested)
                     {
                        _loc6_.validateDisplayList();
                     }
                  }
                  else
                  {
                     _loc7_ = _loc6_.displayObject as ISharedDisplayObject;
                     if(!_loc7_ || _loc7_.redrawRequested)
                     {
                        _loc6_.validateDisplayList();
                        if(_loc7_)
                        {
                           _loc7_.redrawRequested = false;
                        }
                     }
                  }
               }
         ...
         if(this.scaleGridChanged)
         {
            ...
            if(this.isValidScaleGrid())
            {
               _loc8_ = !!mx_internal::_overlay ? int(mx_internal::_overlay.numDisplayObjects) : 0;
               if(numChildren - _loc8_ > 0)
               {
      ...
      {
         var _loc3_:int = !!mx_internal::_overlay ? int(mx_internal::_overlay.numDisplayObjects) : 0;
         if(param1.parent == this)
         {
